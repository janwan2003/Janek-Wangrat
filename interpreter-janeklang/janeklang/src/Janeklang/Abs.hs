...
-- | The abstract syntax of language janeklang.

module Janeklang.Abs where
...
data Program' a = PProg a [TopDef' a]
...
data Arg' a = PAr a (Type' a) Ident | PArRef a (Type' a) Ident
  deriving (C.Eq, C.Ord, C.Show, C.Read, C.Functor, C.Foldable, C.Traversable)

type Block = Block' BNFC'Position
data Block' a = SBlk a [Stmt' a]
  deriving (C.Eq, C.Ord, C.Show, C.Read, C.Functor, C.Foldable, C.Traversable)
...
data Type' a
    = TInt a | TStr a | TBool a | TVoid a | Fun a (Type' a) [Type' a]
  deriving (C.Eq, C.Ord, C.Show, C.Read, C.Functor, C.Foldable, C.Traversable)
